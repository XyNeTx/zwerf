...
class BeefSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbBeef = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_beef_selected)

        recyclerView = findViewById(R.id.recyclerViewBeef)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbBeef = FirebaseFirestore.getInstance()

        dbBeef.collection("MenuBeef").get().addOnSuccessListener {
            if (!it.isEmpty) {

...
class SquidSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbSquid = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_squid_selected)

        recyclerView = findViewById(R.id.recyclerViewSquid)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbSquid = FirebaseFirestore.getInstance()

        dbSquid.collection("MenuSquid").get().addOnSuccessListener {
            if (!it.isEmpty) {

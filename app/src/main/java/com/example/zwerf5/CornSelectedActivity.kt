...
class CornSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbCorn = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_corn_selected)

        recyclerView = findViewById(R.id.recyclerViewCorn)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbCorn = FirebaseFirestore.getInstance()

        dbCorn.collection("MenuCorn").get().addOnSuccessListener {
            if (!it.isEmpty) {

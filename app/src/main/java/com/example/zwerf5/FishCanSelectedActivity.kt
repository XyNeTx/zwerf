...
class FishCanSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbFishCan = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_fish_can_selected)

        recyclerView = findViewById(R.id.recyclerViewFishCan)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbFishCan = FirebaseFirestore.getInstance()

        dbFishCan.collection("MenuFishCan").get().addOnSuccessListener {
            if (!it.isEmpty) {

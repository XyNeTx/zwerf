...
class CabbageSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbCabbage = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_cabbage_selected)

        recyclerView = findViewById(R.id.recyclerViewCabbage)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbCabbage = FirebaseFirestore.getInstance()

        dbCabbage.collection("MenuCabbage").get().addOnSuccessListener {
            if (!it.isEmpty) {

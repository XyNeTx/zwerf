...
class ShrimpSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbShrimp = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_shrimp_selected)

        recyclerView = findViewById(R.id.recyclerViewShrimp)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbShrimp = FirebaseFirestore.getInstance()

        dbShrimp.collection("MenuShrimp").get().addOnSuccessListener {
            if (!it.isEmpty) {

...
class EggSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbEgg = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_egg_selected)

        recyclerView = findViewById(R.id.recyclerViewEgg)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbEgg = FirebaseFirestore.getInstance()

        dbEgg.collection("MenuEgg").get().addOnSuccessListener {
            if (!it.isEmpty) {

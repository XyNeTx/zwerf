...
class StrawberrySelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbStraw = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_strawberry_selected)

        recyclerView = findViewById(R.id.recyclerViewStrawberry)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbStraw = FirebaseFirestore.getInstance()

        dbStraw.collection("MenuStrawberry").get().addOnSuccessListener {
            if (!it.isEmpty) {

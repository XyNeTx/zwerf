...
class PorkSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbPork = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_pork_selected)

        recyclerView = findViewById(R.id.recyclerViewPork)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbPork = FirebaseFirestore.getInstance()

        dbPork.collection("MenuPork").get().addOnSuccessListener {
            if (!it.isEmpty) {

...
class DuckSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbDuck = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_duck_selected)

        recyclerView = findViewById(R.id.recyclerViewDuck)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbDuck = FirebaseFirestore.getInstance()

        dbDuck.collection("MenuDuck").get().addOnSuccessListener {
            if (!it.isEmpty) {

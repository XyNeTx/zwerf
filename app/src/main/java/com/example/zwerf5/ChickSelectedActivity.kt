...
class ChickSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbChick = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_chick_selected)

        recyclerView = findViewById(R.id.recyclerViewChick)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbChick = FirebaseFirestore.getInstance()

        dbChick.collection("MenuChick").get().addOnSuccessListener {
            if (!it.isEmpty) {

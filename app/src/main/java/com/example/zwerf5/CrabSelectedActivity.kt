...
class CrabSelectedActivity : AppCompatActivity(), OnMenuClickListener {

    private lateinit var recyclerView: RecyclerView
    private lateinit var menuList: ArrayList<Menu>
    private lateinit var menuAdapter: MenuAdapter
    private var dbCrab = Firebase.firestore

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_crab_selected)

        recyclerView = findViewById(R.id.recyclerViewCrab)
        recyclerView.layoutManager = LinearLayoutManager(this)

        menuList = arrayListOf()

        dbCrab = FirebaseFirestore.getInstance()

        dbCrab.collection("MenuCrab").get().addOnSuccessListener {
            if (!it.isEmpty) {
